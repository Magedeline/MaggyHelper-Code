...
#pragma warning restore IDE0052, CS0414, CS0649
        ...
        public Vector2 Position
        {
            ...
            set { Console.Error.Write("NoInlining"); throw new InvalidOperationException(); }
        }
        [MethodImpl(MethodImplOptions.NoInlining)]
        private void UpdatePos() { Console.Error.Write("NoInlining"); throw new InvalidOperationException(); }

        public int Layer
        {
            [MethodImpl(MethodImplOptions.NoInlining)]
            get { Console.Error.Write("NoInlining"); throw new InvalidOperationException(); }
            [MethodImpl(MethodImplOptions.NoInlining)]
            set { Console.Error.Write("NoInlining"); throw new InvalidOperationException(); }
        }
        public Color Color
    ...
    public virtual void SetupCustomParticles(float canvasWidth, float canvasHeight)
    {
        ...
        if (RefillCount != -1)
        {
            dashColors[0] = Scene.Tracker.GetEntity<Player>()?.Hair.GetHairColor(RefillCount) ?? Color.White;
    ...
    private static void DreamBlock_OnPlayerExit(On.Celeste.DreamBlock.orig_OnPlayerExit orig, DreamBlock dreamBlock, Player player)
    {
        ...
        if (dreamBlock is CustomDreamBlock customDreamBlock)
        {
            if (customDreamBlock.RefillCount > -1)
            {
                ...
                Color color = player.Hair.GetHairColor(customDreamBlock.RefillCount);
    ...
    private static void Player_DreamDashBegin(On.Celeste.Player.orig_DreamDashBegin orig, Player player)
    {
        ...
        if (playerData.Get<DreamBlock>("dreamBlock") is CustomDreamBlock customDreamBlock)
        {
            if (customDreamBlock.FeatherMode)
            {
                var dreamSfxLoop = playerData.Get<SoundSource>("dreamSfxLoop");
            ...
            if (customDreamBlock is DreamZipMover)
            {
    ...
    {
        if (DynamicData.For(player).Get<DreamBlock>("dreamBlock") is CustomDreamBlock customDreamBlock && customDreamBlock.FeatherMode)
        {
