...
using Celeste.Mod.MaggyHelper.Entities;
using MonoMod.Utils;
...
[CustomEntity("MaggyHelper/DreamTheoCrystal")]
...
internal class DreamTheoCrystal : TheoCrystal
{
    ...
    {
        var theoData = new DynamicData(typeof(TheoCrystal), this);
        Remove(theoData.Get<Sprite>("sprite"));
        Add(dreamSprite = new DreamSprite(CommunalHelperGFX.SpriteBank.Create("dreamTheoCrystal"), particleBounds)
        {
            OnChange = (_, current) => overlaySprite.Play(current, true)
        });
        theoData.Set("sprite", dreamSprite);

        Add(overlaySprite = CommunalHelperGFX.SpriteBank.Create("dreamTheoCrystalOverlay"));

        var origOnPickup = Hold?.OnPickup;
        var origOnRelease = Hold?.OnRelease;
        Remove(Hold);
        ...
        )
        {
            ...
            SlowFall = false,
            SlowRun = true,
            OnPickup = origOnPickup,
            OnRelease = origOnRelease,
