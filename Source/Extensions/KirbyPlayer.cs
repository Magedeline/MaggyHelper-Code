...
using CelesteDreamBlock = Celeste.DreamBlock;
using MaggyHelper.Entities.Kirby;

namespace MaggyHelper.Extensions
{
  /// <summary>
  /// Helper class for converting integers to Facings enum
  /// </summary>
  internal static class ToFacing
  {
    public static Facings Convert(int dir)
    {
      return dir >= 0 ? Facings.Right : Facings.Left;
    }
  }

  // [CustomEntity(ids: "MaggyHelper/KirbyPlayer")]  // Superseded by KirbyPlayerExtension — entity ID now registered there
  [Tracked(false)]
  public class KirbyPlayer : Actor
  {
    ...
    public static readonly Color ThreeDashesHairColor = Color.Orange;
    public static readonly Color FourDashesHairColor = Color.LightBlue;
    public static readonly Color FiveDashesHairColor = Color.Purple;
    public static readonly Color TenDashesHairColor = Color.Gold;
    public static readonly Color NormalBadelineHairColor = BadelineOldsite.HairColor;
    public static readonly Color UsedBadelineHairColor = KirbyPlayer.UsedHairColor;
    public static readonly Color TwoDashesBadelineHairColor = KirbyPlayer.TwoDashesHairColor;
    public static readonly Color ThreeDashesBadelineHairColor = KirbyPlayer.ThreeDashesHairColor;
    public static readonly Color FourDashesBadelineHairColor = KirbyPlayer.FourDashesHairColor;
    public static readonly Color FiveDashesBadelineHairColor = KirbyPlayer.FiveDashesHairColor;
    public static readonly Color TenDashesBadelineHairColor = KirbyPlayer.TenDashesHairColor;
    ...
    private CelesteDreamBlock dreamBlock;
    ...
    public KirbyPlayer(Vector2 position, PlayerSpriteMode spriteMode)
      : base(new Vector2((float) (int) position.X, (float) (int) position.Y))
    {
      ...
      this.Hair.Color = spriteMode != PlayerSpriteMode.MadelineAsBadeline ? KirbyPlayer.NormalHairColor : KirbyPlayer.NormalBadelineHairColor;
      this.startHairCount = this.Sprite.HairCount;
      this.sweatSprite = GFX.SpriteBank.Create("kirby_player_sweat");
      ...
      this.Sprite.OnFrameChange = (Action<string>) (anim =>
      {
        ...
        if (anim.Equals("runSlow_carry") && (currentAnimationFrame == 0 || currentAnimationFrame == 6) || anim.Equals("runFast") && (currentAnimationFrame == 0 || currentAnimationFrame == 6) || anim.Equals("runSlow") && (currentAnimationFrame == 0 || currentAnimationFrame == 6) || anim.Equals("walk") && (currentAnimationFrame == 0 || currentAnimationFrame == 6) || anim.Equals("runStumble") && currentAnimationFrame == 6 || anim.Equals("flip") && currentAnimationFrame == 4 || anim.Equals("runWind") && (currentAnimationFrame == 0 || currentAnimationFrame == 6) || anim.Equals("idleC") && this.Sprite.Mode == PlayerSpriteMode.MadelineNoBackpack && (currentAnimationFrame == 3 || currentAnimationFrame == 6 || currentAnimationFrame == 8 || currentAnimationFrame == 11) || anim.Equals("carryTheoWalk") && (currentAnimationFrame == 0 || currentAnimationFrame == 6) || anim.Equals("push") && (currentAnimationFrame == 8 || currentAnimationFrame == 15))
        {
          ...
          if (platformByPriority != null)
            this.Play("event:/desolozantas/char/kirby/footstep", "surface_index", (float) platformByPriority.GetStepSoundIndex((Entity) this));
        }
        else if (anim.Equals("climbUp") && currentAnimationFrame == 5 || anim.Equals("climbDown") && currentAnimationFrame == 5)
        {
          ...
          if (platformByPriority != null)
            this.Play("event:/desolozantas/char/kirby/handhold", "surface_index", (float) platformByPriority.GetWallSoundIndex(this.GetAssociatedPlayer(), (int) this.Facing));
        }
        else if (anim.Equals("wakeUp") && currentAnimationFrame == 19)
          this.Play("event:/desolozantas/char/kirby/campfire_stand");
        else if (anim.Equals("sitDown") && currentAnimationFrame == 12)
          this.Play("event:/desolozantas/char/kirby/summit_sit");
      ...
      this.Sprite.OnLastFrame = (Action<string>) (anim =>
      {
        if (this.Scene == null || this.Dead || !(this.Sprite.CurrentAnimationID == "idle") || this.level.InCutscene || (double) this.idleTimer <= 3.0 || !Calc.Random.Chance(0.2f))
          return;
        string id = this.Sprite.Mode != PlayerSpriteMode.Madeline ? idleNoBackpackOptions.Choose() : (this.level.CoreMode == Session.CoreModes.Hot ? idleWarmOptions : idleColdOptions).Choose();
        ...
        if (this.Sprite.Mode == PlayerSpriteMode.Madeline)
        {
          ...
          {
            case "idleB":
              this.idleSfx = this.Play("event:/desolozantas/char/kirby/idle_scratch");
              break;
            case "idleC":
              this.idleSfx = this.Play("event:/desolozantas/char/kirby/idle_sneeze");
              break;
          }
        }
        else
        {
          if (!(id == "idleA"))
            return;
          this.idleSfx = this.Play("event:/desolozantas/char/kirby/idle_crackknuckles");
        }
      });
    ...
    /// <summary>
    /// Gets or creates the associated Celeste Player for compatibility with methods expecting Player type.
    /// </summary>
    public Player GetAssociatedPlayer()
    {
      if (_associatedPlayer == null && level != null)
      {
        _associatedPlayer = level.Tracker.GetEntity<Player>();
      }
      return _associatedPlayer;
    }

    /// <summary>
    /// Sets the associated player reference.
    /// </summary>
    ...
    #region KirbyMode Proxy Properties
    // These properties proxy to the associated KirbyMode instance for compatibility

    private KirbyModeExt _kirbyModeExt;

    /// <summary>
    /// Gets the associated KirbyMode instance.
    /// </summary>
    public KirbyModeExt GetKirbyModeExt()
    {
      if (_kirbyModeExt == null && level != null)
      {
        _kirbyModeExt = level.Tracker.GetEntity<KirbyModeExt>();
      }
      return _kirbyModeExt;
    }

    /// <summary>
    /// Whether Kirby is currently inhaling.
    /// </summary>
    public bool IsInhaling => GetKirbyModeExt()?.IsInhaling ?? false;

    /// <summary>
    /// Whether Kirby is currently dashing (uses DashAttacking from base implementation).
    /// </summary>
    public bool IsDashing => this.DashAttacking;

    /// <summary>
    /// Whether Kirby is dead.
    /// </summary>
    public bool IsDead => GetKirbyModeExt()?.IsDead ?? this.Dead;

    /// <summary>
    /// Kirby's current health.
    /// </summary>
    public int CurrentHealth => GetKirbyModeExt()?.CurrentHealth ?? 6;

    /// <summary>
    /// Kirby's maximum health.
    /// </summary>
    public int MaxHealth => GetKirbyModeExt()?.MaxHealth ?? 6;

    /// <summary>
    /// List of entities Kirby has inhaled.
    /// </summary>
    public System.Collections.Generic.List<KirbyActorBase> InhaledEntities => GetKirbyModeExt()?.InhaledEntities ?? new System.Collections.Generic.List<KirbyActorBase>();
    ...
    {
      GetKirbyModeExt()?.Heal(amount);
    }

    /// <summary>
    /// Deal damage to Kirby.
    /// </summary>
    public void TakeDamage(int amount = 1, Microsoft.Xna.Framework.Vector2? source = null)
    {
      GetKirbyModeExt()?.TakeDamage(amount, source);
    }

    /// <summary>
    /// Set Kirby's power state.
    /// </summary>
    public void SetPowerState(KirbyModeExt.KirbyPowerState power)
    {
      GetKirbyModeExt()?.SetPowerState(power);
    }

    /// <summary>
    /// Save Kirby state to session.
    /// </summary>
    public void SaveToSession()
    {
      GetKirbyModeExt()?.SaveToSession();
    }
    ...
    public override void Removed(Scene scene)
    {
      ...
      var player = this.GetAssociatedPlayer();
      foreach (Trigger trigger in this.triggersInside)
      {
        trigger.Triggered = false;
        if (player != null) trigger.OnLeave(player);
      }
    ...
    public override void Render()
    {
      if (SaveData.Instance.Assists.InvisibleMotion && this.InControl && (!this.onGround && this.StateMachine.State != 1 && this.StateMachine.State != 3 || (double) this.Speed.LengthSquared() > 800.0))
        return;
      bool flag = this.Hair != null && this.Hair.Visible;
      if (this.Hair != null)
      ...
      else
      {
        ...
        if (this.Sprite.CurrentAnimationID == "startStarFly")
        {
          ...
          GFX.Game.GetAtlasSubtexturesAt("characters/kirby_player/startStarFlyWhite", this.Sprite.CurrentAnimationFrame).Draw(this.Sprite.RenderPosition, this.Sprite.Origin, this.starFlyColor * num, this.Sprite.Scale, this.Sprite.Rotation, SpriteEffects.None);
        }
        if (flag)
          this.RenderScarf();
        this.Sprite.Scale.X *= (float) this.Facing;
      ...
      if (this.Hair != null)
        this.Hair.Visible = flag;
    }

    private void RenderScarf()
    {
      if (this.Hair == null || this.Hair.Nodes == null || this.Hair.Nodes.Count < 2)
        return;
      bool flag = SaveData.Instance.Assists.DashMode == Assists.DashModes.Infinite && !this.level.InCutscene;
      int scarfSegmentCount = this.GetScarfSegmentCount(flag);
      Color scarfColor = this.GetScarfColor(flag);
      Color color1 = Color.Lerp(scarfColor, Color.Black, 0.2f);
      Vector2 vector2 = this.Hair.Nodes[0];
      float thickness = 3f;
      int count = Math.Min(this.Hair.Nodes.Count - 1, scarfSegmentCount);
      for (int index = 0; index < count; ++index)
      {
        Vector2 a = this.Hair.Nodes[index];
        Vector2 b = this.Hair.Nodes[index + 1];
        Color color2 = scarfColor;
        if (flag)
        {
          float hue = ((float) this.Scene.TimeActive * 0.6f + (float) index * 0.12f) % 1f;
          color2 = Calc.HsvToColor(hue, 0.9f, 1f);
          Vector2 vector2_1 = this.Speed * -0.02f;
          Draw.Line(a + vector2_1, b + vector2_1, color2 * 0.35f, thickness + 1f);
        }
        Draw.Line(a, b, color2, thickness);
        thickness = Math.Max(1f, thickness - 0.4f);
      }
      if (flag)
      {
        float hue = ((float) this.Scene.TimeActive * 0.6f) % 1f;
        color1 = Calc.HsvToColor(hue, 0.9f, 0.8f);
      }
      Draw.Rect(vector2.X - 2f, vector2.Y - 3f, 4f, 2f, color1);
    }

    private int GetScarfSegmentCount(bool infiniteDashes)
    {
      if (infiniteDashes)
        return 10;
      return this.Dashes switch
      {
        0 => 2,
        1 => 3,
        2 => 4,
        ...
        5 => 6,
        10 => 10,
        _ => Math.Min(6, Math.Max(2, this.Dashes + 1))
      };
    }

    private Color GetScarfColor(bool infiniteDashes)
    {
      if (infiniteDashes)
        return Color.White;
      return this.Dashes switch
      {
        0 => Calc.HexToColor("BFDFFF"),
        1 => Calc.HexToColor("FFB3B3"),
        2 => Calc.HexToColor("00FF4C"),
        3 => Color.Yellow,
        4 => Color.Purple,
        5 => Color.Orange,
        >= 10 => Calc.HsvToColor(((float) this.Scene.TimeActive * 0.6f) % 1f, 0.9f, 1f),
        _ => this.OverrideHairColor.HasValue ? this.OverrideHairColor.Value : this.Hair.Color
      };
    }
    ...
    public override void Update()
    {
      ...
      if (this.OnSafeGround)
      {
        ...
        {
          if (player != null && component.Check(player))
          {
      ...
      if (this.StateMachine.State == 5)
        ...
        this.Sprite.HairCount = this.Dashes > 1 ? this.startHairCount + (this.Dashes - 1) : this.startHairCount;
      ...
      if ((this.Sprite.CurrentAnimationID == null || !this.Sprite.CurrentAnimationID.Equals("wallslide") ? 0 : ((double) this.Speed.Y > 0.0 ? 1 : 0)) != 0)
      {
        if (!this.wallSlideSfx.Playing)
          this.Loop(this.wallSlideSfx, "event:/desolozantas/char/kirby/wallslide");
        Platform platformByPriority = SurfaceIndex.GetPlatformByPriority(this.CollideAll<Solid>(this.Center + Vector2.UnitX * (float) this.Facing, this.temp));
        if (platformByPriority != null)
          this.wallSlideSfx.Param("surface_index", (float) platformByPriority.GetWallSoundIndex(this.GetAssociatedPlayer(), (int) this.Facing));
      }
      else
        ...
        foreach (Trigger entity in this.Scene.Tracker.GetEntities<Trigger>())
        {
          if (this.CollideCheck((Entity) entity))
          {
            if (!entity.Triggered)
            {
              ...
              if (player != null) entity.OnEnter(player);
            }
            if (player != null) entity.OnStay(player);
          }
          else if (entity.Triggered)
          {
            this.triggersInside.Remove(entity);
            entity.Triggered = false;
            if (player != null) entity.OnLeave(player);
          }
        }
      }
      ...
      if (!this.Dead && this.StateMachine.State != 21)
      {
        ...
        {
          if (player != null && component.Check(player) && this.Dead)
          {
            this.Collider = collider;
            return;
          }
      ...
      {
        var player = this.GetAssociatedPlayer();
        if (player != null) this.level.EnforceBounds(player);
      }
      ...
      if (this.wasDucking != this.Ducking)
      {
        ...
        if (this.wasDucking)
          this.Play("event:/desolozantas/char/kirby/duck");
        else if (this.onGround)
          this.Play("event:/desolozantas/char/kirby/stand");
      }
      if ((double) this.Speed.X != 0.0 && (this.StateMachine.State == 3 && !this.SwimUnderwaterCheck() || this.StateMachine.State == 0 && this.CollideCheck<Water>(this.Position)))
      {
        if (!this.swimSurfaceLoopSfx.Playing)
          this.swimSurfaceLoopSfx.Play("event:/desolozantas/char/kirby/water_move_shallow");
      }
      else
    ...
    private void CreateTrail()
    {
      ...
      bool rainbowDash = SaveData.Instance.Assists.DashMode == Assists.DashModes.Infinite || this.Dashes >= 10;
      Color trailColor = rainbowDash ? Calc.HsvToColor(((float) this.Scene.TimeActive * 0.6f) % 1f, 0.9f, 1f) : Color.White;
      if (this.Sprite.Mode == PlayerSpriteMode.MadelineAsBadeline)
        TrailManager.Add((Entity) this, scale, rainbowDash ? trailColor : (this.wasDashB ? KirbyPlayer.NormalBadelineHairColor : KirbyPlayer.UsedBadelineHairColor));
      else
        TrailManager.Add((Entity) this, scale, rainbowDash ? trailColor : (this.wasDashB ? KirbyPlayer.NormalHairColor : KirbyPlayer.UsedHairColor));
    }
    ...
    private void UpdateChaserStates()
    {
      ...
      var player = this.GetAssociatedPlayer();
      if (player != null)
        this.ChaserStates.Add(new Player.ChaserState(player));
    ...
    public void UpdateHair(bool applyGravity)
    {
      ...
      else
      {
        ...
        {
          color = KirbyPlayer.FlashHairColor;
        ...
        else if ((double) this.hairFlashTimer > 0.0)
        {
          ...
          color = KirbyPlayer.GetHairColorForDashes(this.Dashes, this.Sprite.Mode == PlayerSpriteMode.MadelineAsBadeline);
    ...
    private static Color GetHairColorForDashes(int dashes, bool isBadeline)
    {
      return dashes switch
      {
        2 => isBadeline ? KirbyPlayer.TwoDashesBadelineHairColor : KirbyPlayer.TwoDashesHairColor,
        3 => isBadeline ? KirbyPlayer.ThreeDashesBadelineHairColor : KirbyPlayer.ThreeDashesHairColor,
        4 => isBadeline ? KirbyPlayer.FourDashesBadelineHairColor : KirbyPlayer.FourDashesHairColor,
        5 => isBadeline ? KirbyPlayer.FiveDashesBadelineHairColor : KirbyPlayer.FiveDashesHairColor,
        >= 10 => isBadeline ? KirbyPlayer.TenDashesBadelineHairColor : KirbyPlayer.TenDashesHairColor,
        _ => isBadeline ? KirbyPlayer.NormalBadelineHairColor : KirbyPlayer.NormalHairColor
      };
    }
          ...
          if (this.DashAttacking)
          {
            if (this.onGround && (double) this.DashDir.Y == 0.0 && !this.Ducking && (double) this.Speed.X != 0.0 && this.moveX == -Math.Sign(this.Speed.X))
            {
              if (this.Scene.OnInterval(0.02f))
                Dust.Burst(this.Position, -1.5707964f, 1);
    ...
    public void Jump(bool particles = true, bool playSfx = true)
    {
      ...
      {
        if (this.launched)
          this.Play("event:/desolozantas/char/kirby/jump_assisted");
        if (this.dreamJump)
          this.Play("event:/desolozantas/char/kirby/jump_dreamblock");
        else
          this.Play("event:/desolozantas/char/kirby/jump");
      }
    ...
    private void WallJump(int dir)
    {
      ...
      if (platformByPriority != null)
      {
        index = platformByPriority.GetWallSoundIndex(this.GetAssociatedPlayer(), -dir);
        this.Play("event:/desolozantas/char/kirby/landing", "surface_index", (float) index);
        if (platformByPriority is CelesteDreamBlock celesteDreamBlock)
          celesteDreamBlock.FootstepRipple(this.Position + new Vector2((float) (dir * 3), -4f));
      }
      this.Play(dir < 0 ? "event:/desolozantas/char/kirby/jump_wall_right" : "event:/desolozantas/char/kirby/jump_wall_left");
      this.Sprite.Scale = new Vector2(0.6f, 1.4f);
      if (dir == -1)
    ...
    private void SuperWallJump(int dir)
    {
      ...
      this.launched = true;
      this.Play(dir < 0 ? "event:/desolozantas/char/kirby/jump_wall_right" : "event:/desolozantas/char/kirby/jump_wall_left");
      this.Play("event:/desolozantas/char/kirby/jump_superwall");
      this.Sprite.Scale = new Vector2(0.6f, 1.4f);
      int index = -1;
      Platform platformByPriority = SurfaceIndex.GetPlatformByPriority(this.CollideAll<Platform>(this.Position - Vector2.UnitX * (float) dir * 4f, this.temp));
      if (platformByPriority != null)
    ...
    private void ClimbJump()
    {
      ...
      if (platformByPriority != null)
        index = platformByPriority.GetWallSoundIndex(this.GetAssociatedPlayer(), (int) this.Facing);
      if (this.Facing == Facings.Right)
      {
        this.Play("event:/desolozantas/char/kirby/jump_climb_right");
        Dust.Burst(this.Center + Vector2.UnitX * 2f, -2.3561945f, 4, this.DustParticleFromSurfaceIndex(index));
      }
      else
      {
        this.Play("event:/desolozantas/char/kirby/jump_climb_left");
        Dust.Burst(this.Center + Vector2.UnitX * -2f, -0.7853982f, 4, this.DustParticleFromSurfaceIndex(index));
      }
    }
    ...
    public PlayerDeadBody Die(Vector2 direction, bool evenIfInvincible = false, bool registerDeathInStats = true)
    {
      ...
      Input.Rumble(RumbleStrength.Light, RumbleLength.Medium);
      var player = this.GetAssociatedPlayer();
      PlayerDeadBody playerDeadBody = player != null ? new PlayerDeadBody(player, direction) : null;
      if (playerDeadBody != null && goldenStrawb != null)
      {
      ...
      if (playerDeadBody != null) this.Scene.Add((Entity) playerDeadBody);
    ...
    {
      var player = this.GetAssociatedPlayer();
      if (player == null || !pickup.Pickup(player))
        return false;
      this.Ducking = false;
      this.Holding = pickup;
    ...
    public void Throw()
    {
      ...
      else
      {
        ...
        this.Play("event:/desolozantas/char/kirby/crystaltheo_throw");
    ...
    private void OnCollideH(CollisionData data)
    {
      ...
      else
      {
        ...
        {
          var player = this.GetAssociatedPlayer();
          DashCollisionResults collisionResults = player != null ? data.Hit.OnDashCollide(player, data.Direction) : DashCollisionResults.NormalCollision;
    ...
    private void OnCollideV(CollisionData data)
    {
      ...
      else
      {
        ...
        if (data.Hit != null && data.Hit.OnDashCollide != null)
        {
          ...
          {
            var player = this.GetAssociatedPlayer();
            if (player != null) data.Hit.OnDashCollide(player, data.Direction);
            return;
          }
        }
          ...
          if (this.StateMachine.State != 1)
          {
            ...
            if (platformByPriority != null)
            {
              ...
              if (index >= 0 && !this.MuffleLanding)
                this.Play((double) this.playFootstepOnLand > 0.0 ? "event:/desolozantas/char/kirby/footstep" : "event:/desolozantas/char/kirby/landing", "surface_index", (float) index);
              if (platformByPriority is CelesteDreamBlock celesteDreamBlock)
                celesteDreamBlock.FootstepRipple(this.Position);
    ...
    private bool DreamDashCheck(Vector2 dir)
    {
      ...
      {
        CelesteDreamBlock dreamBlock = this.CollideFirst<CelesteDreamBlock>(this.Position + dir);
        if (dreamBlock != null)
        {
          if (this.CollideCheck<Solid, CelesteDreamBlock>(this.Position + dir))
          {
            ...
            if (flag1)
            {
              for (int index = -1; index >= -4; --index)
              {
                if (!this.CollideCheck<Solid, CelesteDreamBlock>(this.Position + dir + vector2 * (float) index))
                {
    ...
    private int NormalUpdate()
    {
      ...
      if (this.Holding == null)
      {
        if (Input.GrabCheck && !this.IsTired && !this.Ducking)
        {
          ...
          {
            if (player != null && component.Check(player) && this.Pickup(component))
              return 8;
          }
    ...
    private void ClimbBegin()
    {
      ...
      if (platformByPriority == null)
        return;
      this.Play("event:/desolozantas/char/kirby/grab", "surface_index", (float) platformByPriority.GetWallSoundIndex(this.GetAssociatedPlayer(), (int) this.Facing));
      if (!(platformByPriority is CelesteDreamBlock celesteDreamBlock))
        return;
      celesteDreamBlock.FootstepRipple(this.Position + new Vector2((float) ((int) this.Facing * 3), -4f));
    }
    ...
    private int ClimbUpdate()
    {
      ...
      {
        this.Speed += this.LiftBoost;
        this.Play("event:/desolozantas/char/kirby/grab_letgo");
        return 0;
      }
    ...
    private void ClimbHop()
    {
      ...
      this.Play("event:/desolozantas/char/kirby/climb_ledge");
    }
    ...
    private bool ClimbHopBlockedCheck()
    {
      ...
      if (player != null)
      {
        ...
        {
          if (component.HopBlockCheck(player))
            return true;
        }
      }
    ...
    private bool JumpThruBoostBlockedCheck()
    {
      ...
      if (player != null)
      {
        foreach (LedgeBlocker component in this.Scene.Tracker.GetComponents<LedgeBlocker>())
        {
          if (component.JumpThruBoostCheck(player))
            return true;
        }
      }
      return false;
    ...
    private bool DashCorrectCheck(Vector2 add)
    {
      ...
      if (player != null)
      {
        foreach (LedgeBlocker component in this.Scene.Tracker.GetComponents<LedgeBlocker>())
        {
          if (component.DashCorrectCheck(player))
          {
            this.Position = position;
            this.Collider = collider;
            return true;
    ...
    private void CallDashEvents()
    {
      ...
      if (this.CurrentBooster == null)
      {
        ...
        {
          if (this.wasDashB)
            this.Play("event:/desolozantas/char/kirby/dash_pink_right");
          else
            this.Play("event:/desolozantas/char/kirby/dash_red_right");
        }
        else if (this.wasDashB)
          this.Play("event:/desolozantas/char/kirby/dash_pink_left");
        else
          this.Play("event:/desolozantas/char/kirby/dash_red_left");
        if (this.SwimCheck())
          this.Play("event:/desolozantas/char/kirby/water_dash_gen");
      ...
      else
      {
        var player = this.GetAssociatedPlayer();
        if (player != null)
          this.CurrentBooster.PlayerBoosted(player, this.DashDir);
        this.CurrentBooster = (Booster) null;
      }
    }
    ...
    private IEnumerator DashCoroutine()
    {
      ...
      if (player.onGround && (double) player.DashDir.X != 0.0 && (double) player.DashDir.Y > 0.0 && (double) player.Speed.Y > 0.0 && (!player.Inventory.DreamDash || !player.CollideCheck<CelesteDreamBlock>(player.Position + Vector2.UnitY)))
      {
    ...
    {
      KirbyPlayer player = this;
      player.Play("event:/desolozantas/char/kirby/crystaltheo_lift");
      Input.Rumble(RumbleStrength.Medium, RumbleLength.Short);
      ...
      Vector2 carryOffsetTarget = KirbyPlayer.CarryOffsetTarget;
      Vector2 control = new Vector2(begin.X + (float) (Math.Sign(begin.X) * 2), KirbyPlayer.CarryOffsetTarget.Y - 2f);
    ...
    private void DreamDashBegin()
    {
      ...
      this.Stamina = 110f;
      this.dreamJump = false;
      this.Play("event:/desolozantas/char/kirby/dreamblock_enter");
      this.Loop(this.dreamSfxLoop, "event:/desolozantas/char/kirby/dreamblock_travel");
    }
    ...
    private void DreamDashEnd()
    {
      ...
      if (this.dreamBlock != null)
      {
        if ((double) this.DashDir.X != 0.0)
        {
          ...
          this.jumpGraceTimer = 0.0f;
        var player = this.GetAssociatedPlayer();
        if (player != null) this.dreamBlock.OnPlayerExit(player);
        this.dreamBlock = (CelesteDreamBlock) null;
      }
      this.Stop(this.dreamSfxLoop);
      this.Play("event:/desolozantas/char/kirby/dreamblock_exit");
      Input.Rumble(RumbleStrength.Medium, RumbleLength.Short);
    }
    ...
    private int DreamDashUpdate()
    {
      ...
      CelesteDreamBlock dreamBlock = this.CollideFirst<CelesteDreamBlock>();
      if (dreamBlock == null)
      {
    ...
    private IEnumerator TempleFallCoroutine()
    {
      ...
      while (!player.onGround)
        yield return (object) null;
      player.Play("event:/desolozantas/char/kirby/mirrortemple_big_landing");
      if (player.Dashes <= 1)
    ...
    private IEnumerator IntroJumpCoroutine()
    {
      ...
      {
        yield return (object) 0.2f;
        player.Play("event:/desolozantas/char/kirby/summit_areastart");
    ...
    {
      this.Play("event:/desolozantas/char/kirby/revive");
      this.Depth = -1000000;
      this.introEase = 1f;
    ...
    private IEnumerator BirdDashTutorialCoroutine()
    {
      ...
      if (climbing)
      {
        ...
        Dust.Burst(player.Position + new Vector2(4f, -6f), new Vector2(-4f, 0.0f).Angle(), 1);
        player.Speed.Y = 0.0f;
        yield return (object) 0.2f;
        ...
        player.Y = (float) Math.Round((double) player.Y);
        player.Play("event:/desolozantas/char/kirby/climb_ledge");
      ...
      else
      {
        ...
        while ((double) player.Speed.X != 0.0)
        {
          ...
          if (player.Scene.OnInterval(0.04f))
            Dust.Burst(player.BottomCenter + new Vector2(0.0f, -2f), -2.3561945f, 1);
          yield return (object) null;
        }
      }
    }
    ...
    public class KirbyMode : Actor
    {
      ...
      public override void Added(Scene scene)
      {
        ...
        {
          // TODO: Fix type conflict - nested KirbyMode vs standalone KirbyMode
    ...
    public struct ChaserState
    {
      ...
      public ChaserState(Player player)
      {
        ...
        // Note: activeSounds is private in Player, so we can't access it here
        // Use reflection or skip sounds for KirbyPlayer's ChaserState
        this.Sounds = 0;
        this.sound0 = new Player.ChaserStateSound();
        this.sound1 = new Player.ChaserStateSound();
        this.sound2 = new Player.ChaserStateSound();
        this.sound3 = new Player.ChaserStateSound();
        this.sound4 = new Player.ChaserStateSound();
      }
