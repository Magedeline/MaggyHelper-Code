using MaggyHelper; // For KirbyNPC
...
namespace MaggyHelper.Triggers
{
    ...
    /// </summary>
    [CustomEntity(ids: "MaggyHelper/WindTrigger")]
    [HotReloadable]
    public class WindTrigger : Trigger
    {
        ...
        private void ApplyKirbySpecificEffects(global::Celeste.Player player, Vector2 windForce)
        {
            ...
            // Use Tracker or FindFirst to avoid iterating Scene.Entities during Update
            // (Direct LINQ on Scene.Entities can cause "Collection was modified" errors with mod hooks)
            var kirbyNPC = Scene.Tracker.GetEntity<KirbyNPC>();
            bool hasKirbyEntities = kirbyNPC != null;
